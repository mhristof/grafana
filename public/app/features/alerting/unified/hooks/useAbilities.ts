...
export function useAllAlertmanagerAbilities(): Abilities<AlertmanagerAction> {
  ...
  return getAlertmanagerAbilities({
...
export function getAlertmanagerAbilities({
  alertmanagerName,
  hasConfigurationAPI,
  ...
  isGrafanaFlavoredAlertmanager: boolean;
}): Abilities<AlertmanagerAction> {
  ...
  const notificationsPermissions = getNotificationsPermissions(alertmanagerName);
  const instancePermissions = getInstancesPermissions(alertmanagerName);

  // const silencePermission = isGrafanaFlavoredAlertmanager
  //   ? instancePermissions.create || AccessControlAction.AlertingSilenceCreate
  //   : instancePermissions.create;

  const canCreateSilence = isGrafanaFlavoredAlertmanager
    ? ctx.hasPermission(instancePermissions.create) || ctx.hasPermission(AccessControlAction.AlertingSilenceCreate)
    : ctx.hasPermission(instancePermissions.create);

  // const silencePermission = [AlwaysSupported, isGrafanaFlavoredAlertmanager ?  ctx.hasPermission(action)],
  ...
  return {
    ...
    [AlertmanagerAction.CreateSilence]: [AlwaysSupported, canCreateSilence],
...
function useCanSilence(rule: CombinedRule): [boolean, boolean] {
  ...
  const folderUID = rule && isGrafanaRulerRule(rule.rulerRule) ? rule.rulerRule.grafana_alert.namespace_uid : undefined;
